...
#define TUX3_INOTE_ORPHANED	(1 << 30)
/* the dead flag is set by only backend, so no need per-delta flag */
#define TUX3_INODE_DEAD		(1 << 31)

#define NON_DIRTY_FLAGS		(TUX3_INOTE_ORPHANED | TUX3_INODE_DEAD)
...
{
	tuxnode->flags |= TUX3_INOTE_ORPHANED;
}
				      ...
				      unsigned delta)
{
	...
	/* Check orphan state */
	*orphaned = 0;
	if (idata->i_nlink == 0 && !(tuxnode->flags & TUX3_INOTE_ORPHANED)) {
