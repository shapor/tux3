...
retry:
	...
	/*
	 * FIXME: these inodes don't needed buffer fork. Unlike
	 * this check, we are better to provide the different
	...
	if (tux_inode(inode)->inum != TUX_VOLMAP_INO &&
		...
		page = pagefork_for_blockdirty(page, tux3_get_current_delta());
		if (IS_ERR(page)) {
			int err;
			unlock_page(page);
